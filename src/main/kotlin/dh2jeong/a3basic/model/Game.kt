package dh2jeong.a3basic.model
...
class Game(val dimension: Int, val debug: Boolean = false) {
    ...
    private var shipsPlacedCount = ReadOnlyIntegerWrapper(0)
    val ShipsPlacedCount: ReadOnlyIntegerProperty = shipsPlacedCount.readOnlyProperty
    ...
    fun placeShip(shipType: ShipType, orientation: Orientation, bowX: Int, bowY: Int): Int {
        if ((gameState.value == GameState.HumanSetup) or (gameState.value == GameState.AiSetup)) {
            val shipID = boards[activePlayer]!!.placeShip(shipType, orientation, bowX, bowY)
            shipsPlacedCount.value += 1
            return shipID
        }
